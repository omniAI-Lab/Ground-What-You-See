...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 如果数据中本身就存在Image.image格式的图片，直接加载即可
        if inputs[0]['data_type'] == 'image':
            if "image" in inputs[0]:
                input_copy[0]['content'][0]['image'] = inputs[0]['image']
            else:
        ...
        prompt_inputs = super()._prepare_inputs(prompt_inputs) # 将采样的图片与文本预编码为embedding
        ...
        if self.max_prompt_length is not None: # 根据最长长度进行裁剪
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            prompt_completion_ids = unwrapped_model.generate(**prompt_inputs, generation_config=self.generation_config) # 生成8条响应 torch.Size([8, 743])
        ...
        if inputs[0]['data_type'] == 'video':
            prompt_inputs["pixel_values_videos"] = prompt_inputs["pixel_values_videos"].repeat(len(prompt_completion_ids), 1) # 在行这个维度上重复8次 torch.Size([3840, 1176]) -> torch.Size([3840 * 8, 1176])
        ...
        try: # 计算每个token在policy model下的logps，后面求KL散度需要
            ...
            per_token_logps = per_token_logps[:, prompt_length - 1 :] # 只截取answer部分的logps torch.Size([8, 315])
        except Exception as e:
        ...
        # 随机打乱的情况下的响应都回答正确了，顺序的情况下也回答正确了，这种情况下应该奖励顺序情况???
